...
export default ({ details, features }) => {
  ...
  return (
    <section id="details" aria-labelledby="details-heading" className="mb-5">
      <h2 id="details-heading" className="mb-4">
        <FontAwesomeIcon icon={faListAlt} fixedWidth /> Home Details
      </h2>

      <ul id="home-details" className="pr-3 mb-0">
        {details
          .slice(0, expanded ? details.length : 14)
          .map((detail, index) => (
            <li key={`d${index}`}>
        ...
        {featuresArray
          .slice(
            0,
            expanded ? featuresArray.length : Math.max(0, 14 - details.length)
          )
          .map((feature, index) => (
            <li key={`f${index}`}>
              <b>{capitalize(startCase(feature.label))}:</b>{" "}
