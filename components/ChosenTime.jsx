...
export default ({
      ...
      <Card className={clsx('p-3 mb-5', breakpoint.up.md && 'mx-n3')}>
         <Row className='align-items-center'>
            <Col
               ...
               lg={5}
               xl={4}
               className={clsx(!activeDate && 'text-secondary', breakpoint.down.md && 'mb-3')}>
               <div className={clsx(activeDate && 'text-dark')} style={{ fontSize: '85%' }}>
                  DATE
               </div>
            ...
            <Col
               ...
               sm={3}
               lg={2}
               className={clsx(!activeTime && 'text-secondary', breakpoint.down.md && 'mb-3')}>
            ...
            <Col xs={12} lg={3} xl={4} className={clsx(breakpoint.down.md && 'text-center')}>
               <Button
                  variant='primary'
                  size='lg'
         ...
         {failureMessage && <div className='text-center text-danger mt-2'>{failureMessage}</div>}
      </Card>
   );
   ...
   async function scheduleTour(startTime, durationInHours) {
      ...
      if (startTime < DateTime.local().plus({ hours: 12 })) {
         setFailureMessage('Tours must be scheduled at least 12 hours in advance.');
         setSubmitting(false);

         // Refresh page to disable conflicting times
         router.replace('/listing/[listingId]/tour', `/listing/${router.query.listingId}/tour`);
      }

      try {
         // Send start time and duration through API
         const response = await fetch('/api/tour-schedules', {
            method: 'POST',
         ...
         setFailureMessage("We're experiencing network issues—please try again later.");
         setSubmitting(false);
      }
   }
};
