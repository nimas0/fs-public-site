...
import Message from '../components/chat-ui/Message';
...
const makeSingleMsg = () => {
  const time = `${chance.date({ string: true })} ${chance.hour({
    twentyfour: true,
  })}:${chance.minute()}`;
  return new Message(
    chance.android_id(),
    chance.pickone([chance.sentence(), chance.paragraph({ sentences: 2 })]),
    chance.pickone([user1, user2]),
    time
  );
};
...
export const msgTableHeaders = [
  ...
  },
  {
    ...
    accessor: (d) => (
      <div>
        <Button size='sm' variant='success' className=' mr-2 rounded-0'>
          Open
        </Button>
        <Button size='sm' variant='danger' className=' mr-2 rounded-0'>
          Delete
        </Button>
      </div>
    ),
